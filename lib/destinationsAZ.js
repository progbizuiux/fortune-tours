 ...
 * @param {Set<string>} published  the set publishedCountrySet() in
 *   lib/navigation.js builds from getCountryParams(): "<regionKey>/<slug>"
 *   for every URL that renders plus "canonical:<regionKey>/<slug>" for the
 *   entry's home region. An empty set (Strapi unreachable) degrades every
 *   link to its region page rather than throwing.
 */
export function resolveDestinationHref(destination, published) {
  /* The canonical page first, then any live alias — a country renders under
     every region that lists it, and with no region in view the one its entry
     calls home is the right link (it is also what the canonical tag on every
     alias points at). Between that and the region fallback sits the
     /destinations/[slug] page, for the few names that have one (India) — the
     same order the navbar's rows resolve in, so the two never disagree. */
  const live = liveCountryHref(
    destination.regions.map((region) => ({
      key: region.key,
      href: region.countryHref,
    })),
    destination.slug,
    published,
  );
  ...
  return (
    live ??
    destinationPageHref(destination.name) ??
    destination.regions[0].regionHref
  );
}

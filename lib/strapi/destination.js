...
export async function getDestinationPage(region) {
  ...
  return {
    hero: normaliseHero(entry?.heroSection, region, Boolean(entry?.highlightsSection || true)),
...
function normaliseHero(block, region, hasHighlights = true) {
  ...
  return {
    ...
    ctas: [
      cta(
        ...
        hasHighlights ? "#curated" : `/search?term=${encodeURIComponent(region.label)}`,
      ),
      cta(
...
function normalisePackages(section) {
  ...
  return {
    ...
    items: cards(list(section.packages), (pkg, i) => ({
      ...
      /* Where the card's "View Itinerary" goes — the same field the country
         entries carry, so the two pages read one schema. */
      href: text(pkg.link),
      image: mediaUrl(
        pkg.image,
        FALLBACK_CARD_IMAGES[i % FALLBACK_CARD_IMAGES.length],

...
// Navbar menus: rows, cards and tiles rise in one after another as a menu
// opens. The keyframe (`--animate-menu-row` in app/globals.css) reads its delay
// from --row-delay, so each item only has to carry its own offset inline — one
// class, no per-index class names. Capped so a long list (Europe has 29
// countries) finishes settling in the same beat as a short one instead of
// trickling in for a second. Shared by the desktop sheets and the mobile
// drill-down so they move identically.
export const MENU_ROW_ENTER = "motion-safe:animate-menu-row";

export function menuRowDelay(index, { step = 24, cap = 240 } = {}) {
  return { "--row-delay": `${Math.min(index * step, cap)}ms` };
}

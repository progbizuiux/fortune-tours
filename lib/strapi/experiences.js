...
const EXPERIENCES_QUERY = {
  populate: {
    ...
    monthsSection: { populate: { months: { populate: { cards: { populate: "*" } } } } },
  },
...
export function normaliseExperiences(data) {
  const items = list(data)
    .map((experience) => {
      ...
      const months = normaliseMonths(experience.monthsSection, placeholders.hero);
...
function normaliseMonths(section, heroFallback) {
  if (!section) return { eyebrow: "", title: "", description: "", tabs: [], cards: {} };
  ...
  list(section.months).forEach((tab, index) => {
    ...
    cards[key] = list(tab.cards).map((card) => {
      ...
      return {
        ...
        href: text(card.link, ""),
        image: mediaUrl(card.image, MONTH_CARD_IMAGES[slugify(title)] ?? heroFallback),
      };
    });
  });

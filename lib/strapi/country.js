...
export function normaliseCountry(entry, cardImage, continentSlug, homeRegion) {
  ...
  const packages = normalisePackages(entry?.packagesSection);
  const hasPackages = Boolean(packages?.items?.length);
  ...
  return {
    ...
    hero: normaliseHero(entry?.heroSection, cardImage, hasPackages),
...
function normaliseHero(section, cardImage, hasPackages = false) {
  if (!section) return {};

  const primaryLink = text(section.primaryCtaLink);
  const defaultTarget = hasPackages ? "#packages" : "#itinerary";
  const resolvedPrimaryLink = primaryLink === "/itinerary" ? defaultTarget : primaryLink;
  ...
  return {
    ...
    ctas: [
      cta(section.primaryCtaLabel, resolvedPrimaryLink, defaultTarget),
      cta(section.secondaryCtaLabel, section.secondaryCtaLink, "/plan-my-trip"),
    ].filter(Boolean),
  };
}
...
function normaliseGround(section) {
  ...
  return {
    ...
    ctaHref: text(section.ctaLink) || text(section.ctaHref),

...
export async function getSearchResults({ pkg, ...filters } = {}) {
  ...
  let results = matches.map((country, index) => {
  ...
  if (filters.style === "cruise") {
    const featured = [
      {
        slug: "china-cruise",
        name: "China Cruise",
        description: "Yangtze • Coastal routes — Asia",
        image:
          heroImages.get("asia/china-cruise") ??
          cardImages.get("asia/china-cruise") ??
          CRUISE_FALLBACK_IMAGES["china-cruise"],
        href: "/asia/china-cruise",
      },
      {
        slug: "royal-caribbean",
        name: "Royal Caribbean",
        description: "Islands • Ocean luxury — Caribbean",
        image:
          heroImages.get("caribbean/royal-caribbean") ??
          cardImages.get("caribbean/royal-caribbean") ??
          CRUISE_FALLBACK_IMAGES["royal-caribbean"],
        href: "/caribbean/royal-caribbean",
      },
      {
        slug: "mediterranean",
        name: "Mediterranean",
        description: "Historic ports & coasts — Asia",
        image:
          heroImages.get("asia/mediterranean") ??
          cardImages.get("asia/mediterranean") ??
          CRUISE_FALLBACK_IMAGES["mediterranean"],
        href: "/asia/mediterranean",
      },
      {
        slug: "cordelia-cruise",
        name: "Cordelia Cruise",
        description: "Coastal voyages • Open waters — North America",
        image:
          heroImages.get("north-america/cordelia-cruise") ??
          cardImages.get("north-america/cordelia-cruise") ??
          CRUISE_FALLBACK_IMAGES["cordelia-cruise"],
        href: "/north-america/cordelia-cruise",
      },
    ];

    const filteredFeatured = featured.filter((item) => {
      if (filters.country && filters.country !== item.name) return false;
      if (filters.continent) {
        const continentMap = {
          "china-cruise": "Asia",
          "royal-caribbean": "Caribbean",
          "mediterranean": "Asia",
          "cordelia-cruise": "North America",
        };
        if (continentMap[item.slug] !== filters.continent) return false;
      }
      if (filters.term) {
        const term = filters.term.toLowerCase();
        const text = `${item.name} ${item.description}`.toLowerCase();
        if (!term.split(/\s+/).every((w) => text.includes(w))) return false;
      }
      return true;
    });

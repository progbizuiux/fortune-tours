import { DESTINATION_REGIONS, destinationPageHref } from "@/lib/navigation";
...
export function resolveDestinationHref(destination, published) {
  ...
  /* Between the country page and the region fallback sits the
     /destinations/[slug] page, for the few names that have one (India) —
     the same order the navbar's rows resolve in, so the two never disagree. */
  return (
    live?.countryHref ??

...
const KERALA_QUERY = {
  populate: {
    ...
    /* Names seasons.cards.image explicitly. This was "*" while the panel
       carrying kerala-season-card was still unreleased — Strapi answers a
       populate path whose field it does not have with a 400 that takes the
       whole page down, so the safe rule asked only for whatever the season
       held. The cost was that "*" populates one level only: card media never
       came through and every card fell back to the pictures the section
       shipped with, which is how India's Kashi Ayodhya came to be illustrated
       by a Kerala hillside.

       That backend is deployed now — the path returns 200 and the cards carry
       their uploads — so the rule can ask for them. */
    timingSection: {
      populate: { seasons: { populate: { cards: { populate: "*" } } } },
    },
  },
};
  ...
  return {
    ...
    items: places.length
      ? places.map((place, i) => ({
          ...
          href: text(place.link) || text(place.href) || undefined,
          image: mediaUrl(
            place.image,
...
function normaliseHighlights(section) {
  ...
  const blocks = [1, 2, 3].map((n) => {
    const block = {
      ...
      ctaLabel: text(section[`ctaLabel${n}`]),
      ctaHref: text(section[`ctaLink${n}`]) || text(section[`ctaHref${n}`]),
    };
    return block.eyebrow || block.title || block.description || block.ctaLabel || block.ctaHref ? block : null;
  });

  ...
  const items = list(data)
    .map((experience) => {
      ...
      return {
        ...
        /* Fallback must be a real route: there is no /concierge page, so an
           experience with no CMS ctaLink sent "Make It Yours" to a 404. The
           trip planner is where that intent leads. */
        ctaHref: text(experience.ctaLink, "/plan-my-trip"),
        ...
        packagesCtaHref:
          text(packages.ctaLink) && text(packages.ctaLink) !== "/concierge"
            ? text(packages.ctaLink)
            : "#seasons",
...
/* `destinationsSection.destinations` drives the card carousel. */
function normaliseDestinations(destinations, fallbacks = []) {
  return list(destinations).map((destination, index) => {
    ...
    return {
      ...
      href: text(destination.link) || text(destination.href) || undefined,

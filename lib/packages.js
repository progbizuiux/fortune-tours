...
export async function getPackage(destination, slug) {
  ...
  try {
    ...
    /* A 403 is the API token lacking `find` on the packages collection —
       the state the panel is in today, and the reason this page answered 500.
       Where the design's own copy exists below, it is the honest page and is
       served. Where it does not, the error still propagates: a package that
       lives only in the CMS must not be cached as a 404 for an hour because
       of a permission slip — ISR keeps the last good render instead. */
    const forbidden = error instanceof StrapiError && error.status === 403;
    if (!(forbidden && local)) throw error;
    entry = null;
  }

...
import { cta, list, text } from "./normalise";
...
export function normalisePackage(entry) {
  return {
    ...
    health: normaliseHealth(entry?.healthSection ?? entry?.healthNoteSection),
...
function normaliseHealth(section) {
  if (!section) return undefined;

  const rawFeatures = list(section.features ?? section.items ?? section.notes ?? section.reasons);
  ...
  return {
    ...
    features: rawFeatures.length
      ? rawFeatures.map((item) => ({
          title: text(item.title ?? item.heading ?? item.name ?? (typeof item === "string" ? item : item.text ?? item.note), ""),
          body: text(item.body ?? item.description ?? item.subtitle),
        }))
      : undefined,
  };
}

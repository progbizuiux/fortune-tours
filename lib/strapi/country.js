...
export async function getCountryPage(continentSlug, slug) {
  ...
  return entry ? normaliseCountry(entry) : null;
}
...
export function normaliseCountry(entry) {
  return {
...
function normaliseHero(section) {
  ...
  return {
    ...heading(section),
    image: mediaUrl(section.image, COUNTRY_DEFAULTS.hero),
...
function normaliseFaq(section) {
  ...
  return {
    ...
    /* [] rather than undefined: FaqSection defaults `faqs` to [] and maps over
       it, so an empty accordion is the correct empty state. */

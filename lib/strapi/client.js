...
/* Set on the first 401. Checked before sending, not just retried after: a
   response Next cached while the token still worked is served stale under the
   same token, and its background refresh 401s forever without reaching us. */
...
export async function strapiFetch(path, opts = {}) {
    ...
    try {
      return await fetch(url, {
        headers: {
          ...
          ...(withToken ? { Authorization: `Bearer ${API_TOKEN}` } : {}),
        },
  ...
  const useToken = Boolean(API_TOKEN) && !tokenRejected;
  let res = await send(useToken);

  /* An expired or revoked token fails every request with 401, even ones the
     Public role may read. Drop it and retry so the site keeps rendering, and
     say so once so the token still gets replaced. */
  if (res.status === 401 && useToken) {
    if (!tokenRejected) {
      tokenRejected = true;
      const reason = await res.text().catch(() => "");
      console.warn(
        `[strapi] STRAPI_API_TOKEN was rejected (401 ${reason.slice(0, 120)}); ` +
          "using public access until restart. Regenerate the token in Strapi → Settings → API Tokens.",
      );
    }

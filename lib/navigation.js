...
// The destinations that have a page of their own, keyed by the name the menus
// and the featured cards print. For a /destinations/[slug] page the slug has to
// match what destinationSlug() in lib/strapi/kerala.js derives from the CMS
// entry, because that route sets dynamicParams = false — a slug with no entry
// behind it 404s instead of rendering. Add a line here when a page goes live.
//
// Cordelia Cruise is not a country, so nothing in DESTINATION_REGIONS resolves
// it and the home page's cruise card was falling through to a search for its
// name. Its page lives under North America, and naming it here is what points
// the card at it.
//
// Spain used to need a line here too, papering over BUG-032: getCountryParams()
// and fetchPublishedCountries() only ever read Strapi's first 100 countries, so
// every entry past that silently read as unpublished and fell back to its
// region page. Fixed at the source in lib/strapi/client.js (strapiFetchAll) —
// a real country now only belongs here if it is not in DESTINATION_REGIONS at
// all, the way Cordelia Cruise isn't.
const DESTINATION_PAGES = {
  India: "/destinations/india",
  Kerala: "/destinations/kerala",
  Kashmir: "/destinations/kashmir",
  "Cordelia Cruise": "/north-america/cordelia-cruise",
};
...
/* The reverse: the printed name behind a /destinations/[slug] page, so that
   page can look itself up in lists keyed by name (lib/countryPlaces.js). */
export function destinationNameForSlug(slug) {
  if (!slug) return undefined;
  const href = `/destinations/${slug}`;
  const match = Object.keys(DESTINATION_PAGES).find(
    (name) => DESTINATION_PAGES[name] === href,
  );
...
/* The group's booking engine. Its products are tabs of one JSF page, but a
   `serviceDataID` in the query opens the page on a given tab — 9 is Visa,
   7 is Insurance (both checked against the live portal). Flights and hotels
   are not on this portal at all; they run on the older fortunetours.com
   engines, one page each. */
const PORTAL = "https://travel.fortunetours.in/travel/travel_web.xhtml";

export const BOOKING_LINKS = {
  flight: "https://fortunetours.com/index.php",
  hotel: "https://fortunetours.com/index-hotel.php",
  visa: `${PORTAL}?serviceDataID=9`,
  insurance: `${PORTAL}?serviceDataID=7`,
};
...
export const SITE_MENU = {
  ...
  secondary: [
    /* Every service row books online — see BOOKING_LINKS above for which
       engine each one lands on. */
    { label: "Visa assistance", href: BOOKING_LINKS.visa, external: true },
    { label: "Hotel bookings", href: BOOKING_LINKS.hotel, external: true },
    { label: "Flight booking", href: BOOKING_LINKS.flight, external: true },
    {
      label: "Travel insurance",
      href: BOOKING_LINKS.insurance,
      external: true,
    },
  ],
};
...
const RAW_DESTINATION_REGIONS = [
  ...
  },
  {
    ...
    countries: [
      ...
      country("China Cruise", "Yangtze • Coastal routes"),
      country("Mediterranean", "Historic ports & coasts"),
  ...
  },
  {
    ...
    countries: [
      ...
      country("Royal Caribbean", "Islands • Ocean luxury"),
  ...
  },
  {
    ...
    countries: [
      ...
      country("Cordelia Cruise", "Coastal voyages • Open waters"),
      country("Mexico", "Yucatán • Oaxaca"),
...
/* Where the CMS slug is not what slugify() makes of the printed name. The
   Strapi entry is the URL, so the row has to follow it: "The Maldives" is
   published as `maldives` and "Réunion Island" as `reunion`. Add a line when
   an editor publishes a country under a slug the name would not produce —
   the symptom is a navbar row that falls back to its region page while the
   country page exists. */
const SLUG_OVERRIDES = {
  ...
  "RAcunion Island": "reunion",
  "Ecuador & the Galapagos": "ecuador-galapagos",
  "Ecuador and the Galapagos": "ecuador-galapagos",
};

const countrySlug = (name) => SLUG_OVERRIDES[name] ?? slugify(name);
...
export const DESTINATION_REGIONS = RAW_DESTINATION_REGIONS.map((region) => ({
  ...
  countries: region.countries.map((c) => ({
    ...
    href:
      destinationPageHref(c.name) || `/${region.key}/${countrySlug(c.name)}`,
  })),
}));
 ...
 *   1. the page under the region the reader is looking at, if it renders
 *      (a published country renders under every region that lists it, so
 *      Indian Ocean → Mauritius stays at /indian-ocean/mauritius);
 *   2. else the country's canonical page, or any live alias of it, under
 *      another region — only reached when the browsed region is not one the
 *      page answers under;
...
/* The same rule for a name on its own, with no region in view — the curated
   cards. The canonical page wins, then any live alias, then the destination
   page, then the first region's page. */
export function resolveCountryHrefByName(name, published) {
  const entries = REGIONS_BY_COUNTRY.get(name) ?? [];

  return (
    liveCountryHref(entries, countrySlug(name), published) ??
...
/* A country's page from its printed name — "Greece" → /europe/greece.
   The row's own href, so a card naming a country lands exactly where the
   Destinations menu sends that country, rather than on a search for its name.
   Undefined for a name that is not a country in DESTINATION_REGIONS, which is
   what lets a caller fall back. */
export function countryHrefByName(name) {
  if (!name) return undefined;
  return REGIONS_BY_COUNTRY.get(name)?.[0]?.href ?? destinationPageHref(name);
}

/* Every region key a country slug is listed under — ["africa", "asia",
   "indian-ocean"] for `mauritius`. This is what makes /indian-ocean/mauritius
   a real URL: a country page answers under EVERY region that lists it, not
   only the one its Strapi entry names, so a reader browsing the Indian Ocean
   stays in the Indian Ocean. See getCountryPage() in lib/strapi/country.js. */
const REGION_KEYS_BY_SLUG = new Map();
for (const region of DESTINATION_REGIONS) {
  for (const place of region.countries) {
    const keys = REGION_KEYS_BY_SLUG.get(place.slug) ?? [];
    keys.push(region.key);
    REGION_KEYS_BY_SLUG.set(place.slug, keys);
  }
}

export function regionKeysForCountry(slug) {
  return REGION_KEYS_BY_SLUG.get(slug) ?? [];
}

/* The region a country page canonically lives under: the one its Strapi entry
   names, or — for an entry whose field is blank — the first region the navbar
   lists it in, so the choice is deterministic rather than "whichever alias
   was requested first". Shared by lib/strapi/country.js and
   lib/strapi/search.js so they cannot disagree. */
export function homeRegionForCountry(slug, continentSlug) {
  return continentSlug || regionKeysForCountry(slug)[0] || undefined;
}

/* The key resolvers look for when they want the canonical URL rather than
   any live alias. */
const canonicalKey = (regionKey, slug) => `canonical:${regionKey}/${slug}`;

/* The published set as the menus want it, from getCountryParams()'s
   [{ slug: region, country, canonical }] rows: "<region>/<slug>" for every
   URL that renders, plus "canonical:<region>/<slug>" for the one the entry
   calls home. Rows without the flag (an older caller) contribute the plain
   key only. Tolerates undefined so a menu can render before the layout has
   anything to give it. */
export function publishedCountrySet(params) {
  const set = new Set();
  for (const entry of params ?? []) {
    set.add(`${entry.slug}/${entry.country}`);
    if (entry.canonical) set.add(canonicalKey(entry.slug, entry.country));
  }
  return set;
}

/* Among a name's regions, the href of its canonical page, else of any live
   page, else undefined — the shared "no region in view" rule for the curated
   cards, the A to Z and /search. */
export function liveCountryHref(entries, slug, published) {
  const canonical = entries.find((entry) =>
    published?.has(canonicalKey(entry.key, slug)),
  );
  const live =
    canonical ??
    entries.find((entry) => published?.has(`${entry.key}/${slug}`));
  return live?.href;
}
...
export const CONCIERGE_PROMO = {
  ...
  phoneLabel: "Call +91 96562 11888",
  phoneHref: "tel:+919656211888",
...
export const EXPERIENCE_MENU = [
  {
    ...
    image: "/experiance-menu/family2.png",
  ...
  },
  {
    ...
    image: "/experiance-menu/honeymoon2.png",
  ...
  },
  {
    ...
    image: "/experiance-menu/luxury.png",
  ...
  },
  {
    ...
    image: "/experiance-menu/adventure2.png",
  ...
  },
  {
    ...
    image: "/experiance-menu/Spiritual2.png",

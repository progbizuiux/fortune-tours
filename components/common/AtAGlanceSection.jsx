...
export function AtAGlanceSection({
  ...
  return (
    <section className={cn("bg-cream py-16 md:py-24 lg:py-[100px] relative z-10", className)}>
      <Container>
        <SectionHeading
          align="center"
          eyebrow={eyebrow}
        ...
        <div className="mt-16 md:mt-20 lg:mt-[95px] w-full max-w-[1920px] mx-auto overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden pb-4">
          <ul className="flex min-w-max xl:min-w-0 xl:grid xl:grid-cols-6 divide-x divide-black/10">
            {stats.map((stat, i) => (
              ...
              >
                <h4 className="font-sans text-[15px] xl:text-[16px] font-medium text-black mb-3">
                  {stat.label}
                </h4>
                <p className="font-sans text-[13px] xl:text-[14px] font-light text-black/70 leading-[1.6] whitespace-pre-line">
                  {stat.value}
                </p>
              </li>
            ))}

...
export function destinationOptionsFor({ regionKey, country } = {}) {
  ...
  if (places) return limitDestinationOptions(places);
  ...
  return limitDestinationOptions(ordered);
}
...
export const planSchema = z
  .object({
    destinationMode: z.string().optional(),
    destination: z.string().trim().min(1, "Tell us where you'd like to go or choose an option."),
  ...
  .superRefine((values, ctx) => {
    ...
    if (values.arriving) {
      if (!DATE_RE.test(values.arriving)) {
        ctx.addIssue({
          code: "custom",
          path: ["arriving"],
          message: "Enter a valid date.",
        });
      } else if (values.arriving < today) {
        ctx.addIssue({
          code: "custom",
          path: ["arriving"],
          message: "Arrival date must be in the future.",
        });
      } else if (values.arriving > maxAhead) {
        ctx.addIssue({
          code: "custom",
          path: ["arriving"],
    ...
    if (values.returning) {
      if (!DATE_RE.test(values.returning)) {
        ctx.addIssue({
          code: "custom",
          path: ["returning"],
          message: "Enter a valid date.",
        });
      } else if (values.returning < today) {
        ctx.addIssue({
          code: "custom",
          path: ["returning"],
          message: "Return date must be in the future.",
        });
      } else if (values.arriving && values.returning < values.arriving) {
        ctx.addIssue({
          code: "custom",
          path: ["returning"],
          message: "The return date can't be before arrival.",
        });
      } else if (values.returning > maxAhead) {
        ctx.addIssue({
          code: "custom",
          path: ["returning"],
          message: "Return date is too far in the future.",
        });
      }
    }
  });

...
/* Whether a picked package passes the filters, judged by the country or
   region its URL sits under (/destinations/<destination>/<slug>). `region` is
   that destination's CMS region when it is not in the catalogue (goa → india). */
export function packageMatches(pack, { term, continent, country }) {
  ...
  const labels = home
    ...
    : [regionLabel(pack.region ?? pack.destination)].filter(Boolean);

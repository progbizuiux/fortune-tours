import { StrapiError } from "./strapi/client";
...
const PACKAGES = {
  "kerala/lakshadweep-agatti-kalpitti": {
    ...
    intro: {
      ...
      slides: [
        ...
        },
        {
          ...
          imageAlt: "Kalpitti island sandbank and turquoise lagoon",
          description:
            "Kalpitti Island offers an untouched, uninhabited sanctuary surrounded by crystal waters and vibrant marine life. Wander along pristine sandbanks and snorkel over thriving coral reefs where sea turtles glide peacefully in the warm lagoon.",
          stats: [
            { label: "Duration", value: "4 Days, 3 Nights" },
            { label: "Route", value: "Cochin to Agatti to Cochin" },
            { label: "Islands", value: "Agatti and Kalpitti Islands" },
        ...
        {
          image: "/experiance/maldives.png",
          imageAlt: "Aerial view of Agatti reef and turquoise lagoon",
          description:
            "From tranquil sunrise walks along the reef to glass-bottom boat rides revealing colourful marine sanctuaries, every moment is crafted to immerse you in Lakshadweep's untouched coastal magic.",
          stats: [
            { label: "Duration", value: "4 Days, 3 Nights" },
            { label: "Route", value: "Cochin to Agatti to Cochin" },
            { label: "Islands", value: "Agatti and Kalpitti Islands" },
    ...
    whyUs: {
      title: "Health Note Before You Travel",
      features: [
        {
          title: "A Yellow Fever vaccination is required for travel to Africa",
        },
        {
          title: "Take the vaccine before departure",
        },
        {
          title: "Carry the official vaccination certificate with you through the whole tour",
        },
        {
          title: "Airlines and immigration may ask to see it, so keep it easy to reach",
        },
      ],
    },
    ...
    faq: {
      ...
      contactInfo:
        "Still have a question?\nCall 9656 211 888 or write to info@fortunetours.com",
      faqs: [
        {
    ...
    /* The closing "hold your seat" band between the cancellation table and
       the FAQ (components/packages/BookingCtaSection). The first CTA opens the
       enquiry dialog rather than navigating — see `opensForm` in the hero. */
    cta: {
      eyebrow: "Fixed Departure",
      title: "Hold your seat for\nAgatti & Kalpitti",
      description:
        "Four days, three nights, with flights, stay, meals and the permit included. Seats are limited on every departure — tell us your dates and we confirm availability within a day.",
      ctas: [
        { label: "Book Your Seat", href: "/plan-my-trip", opensForm: true },
        { label: "Call 9656 211 888", href: "tel:+919656211888" },
      ],
      image: "/experiance/beach-escape.png",
      imageAlt:
        "A lighthouse surrounded by palm trees on a headland above clear blue water",
      packageName: "Lakshadweep Package Agatti & Kalpitti",
    },
  },
};
 ...
 *
 * THE CMS DECIDES whether the page exists. This used to read the map first and
 * 404 on a URL missing from it, even when Strapi had the entry published —
 * which made every newly added package unreachable until someone typed a key
 * in here and redeployed. The map is a fallback for copy, not an allowlist for
 * routes, so the lookup below is what answers "is there a page" and the map
 * only fills the gaps underneath it. A package the map has and the CMS does
 * not is still served, so the entry below keeps working exactly as it does now.
 */
export async function getPackage(destination, slug) {
  ...
  let entry;
  try {
    entry = await getPackageEntry(destination, slug);
  } catch (error) {
    /* A 403 is Strapi's Public role lacking `find`/`findOne` on the packages
       collection — the state the panel is in today (Settings → Users &
       Permissions → Roles → Public → Package). Read as "no CMS entry": where
       the design's own copy exists below it is served, and where it does not
       the route 404s, which is the honest answer while the collection is
       unreadable — a crash on every package link was the alternative. The
       moment the permission is granted, the publish webhook (or the hourly
       window) regenerates these pages with the CMS content. Every other
       failure still propagates so ISR keeps the last good render. */
    const forbidden = error instanceof StrapiError && error.status === 403;
    if (!forbidden) throw error;
    console.warn(
      `[packages] Strapi refused the packages collection (403) for ` +
        `${destination}/${slug}; serving ${local ? "the local copy" : "404"}. ` +
        `Grant the Public role find/findOne on Package in Strapi.`,
    );
    entry = null;
  }
  ...
  return {
    ...
    whyUs: entry.whyUs ?? local?.whyUs,
    cancellation: merge(local?.cancellation, entry.cancellation),
    faq: merge(local?.faq, entry.faq),
    /* The closing "hold your seat" band (components/packages/BookingCtaSection).
       Merged like every other section, so the design's copy below stands in
       until the CMS ctaSection is filled — and so the band does not vanish
       with the CMS entry. */
    cta: merge(local?.cta, entry.cta),
  };
}

 ...
 * banner at the top of /search is themed from. The countries and continents
 * an editor picks on that entry decide the filter (see getSearchResults() in
 * lib/strapi/search.js); the lists below only apply to a style with nothing
 * picked yet. A country name listed under a type matches that filter, and a
 * country under none of them only appears when no type is set.
 *

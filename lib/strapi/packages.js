  ...
  populate: {
    ...
    introSection: {
      populate: {
        image: true,
        stats: true,
        slides: { populate: { image: true, stats: true } },
      },
    },
...
function normaliseIntro(section) {
  ...
  const slides = cards(list(section.slides), (slide) => ({
    title: text(slide.title),
    image: mediaUrl(slide.image) ?? undefined,
    imageAlt: mediaAlt(slide.image) || undefined,
    description: text(slide.description),
    stats: cards(list(slide.stats), (stat) => ({
      label: text(stat.label, ""),
      value: text(stat.value, ""),
    })),
    places: splitLines(slide.places),
  }));
...
function normaliseInclusions(section) {
  ...
  const coveredLabel = text(section.coveredLabel);
  const notCoveredLabel = text(section.notCoveredLabel);

  const tabs = [
    {
      key: "covered",
      label: coveredLabel === "Covered" ? "Inclusions" : (coveredLabel || "Inclusions"),
      title: text(section.coveredTitle),
      items: splitLines(section.coveredItems),
    },
    {
      key: "not-covered",
      label: notCoveredLabel === "Not Covered" ? "Exclusions" : (notCoveredLabel || "Exclusions"),
      title: text(section.notCoveredTitle),
      items: splitLines(section.notCoveredItems),
    },
    /* A tab with no label cannot be pressed, and a tab with no items has
       nothing to show — drop either rather than render an empty panel. */
  ].filter((tab) => tab.label && tab.items.length);

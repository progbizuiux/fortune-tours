...
import { cta, internalHref, list, slugify, text } from "./normalise";
...
function normaliseHero(block) {
  ...
  return {
    ...
    description: text(block.description),
    ctas: [
      cta(block.primaryCtaLabel, block.primaryCtaLink, "/plan-my-trip"),
      cta(block.secondaryCtaLabel, block.secondaryCtaLink, "/destinations/a-z"),
    ].filter(Boolean),
  ...
  const slides = tabs.flatMap((tab, i) =>
    list(tab.slides).map((slide, j) => {
      ...
      return {
        ...
        ctaHref: internalHref(slide.buttonLink, ""),
  ...
  withSailings.forEach((month, i) => {
    ...
    cardsData[key] = list(month.departures).map((departure, j) => ({
      ...
      meta: [
        ...
        .filter(Boolean)
        .join(" - "),
      href: internalHref(departure.link),
      image: mediaUrl(
        departure.image,
    ...
    items: photos.length
      ? photos.map((photo, i) => {
          ...
          return {
            ...
            alt: hasImage
              ? mediaAlt(photo.image, text(photo.text, ""))
              : fallback.alt,

...
/**
 * A { label, href } pair, or null when there is no label OR no href.
 *
 ...
 *
 * A label with no href and no fallback returns null rather than an href-less
 * pair, because next/link throws on `href={undefined}` — during a build that
 * aborts the export ("Cannot destructure property 'auth' of 'a'"), and on a
 * live server it is a 500 for the whole page. One empty CMS link field took
 * /africa/congo down that way. Dropping the button degrades the page; keeping
 * it deletes the page.
 */
export function cta(label, href, fallbackHref) {
  ...
  const target = text(href, fallbackHref);
  if (!target) return null;

  return { label: resolved, href: target };
}

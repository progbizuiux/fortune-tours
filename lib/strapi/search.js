...
import { getTravelStyleDestinations } from "./travel-styles";
...
export async function getSearchResults({ pkg, ...filters } = {}) {
  /* Not swallowed like the two reads beside it: those only cost pictures and
     ...
     out. */
  const [published, cardImages, picked] = await Promise.all([
  ...
  let matches;
  if (picked) {
    matches = pickStyleCountries(picked, { ...filters, packageKeys });
  ...
  return matches.map(({ country, entry }, index) => {

...
/* Neutral branded placeholder for any destination that has no CMS image yet.
   A single destination-agnostic image avoids the misleading effect of showing,
   say, the Matterhorn for the British Virgin Islands or a Japanese pagoda for
   Ecuador. Editors should upload a real photo; this just keeps the card
   presentable until they do. */
const FALLBACK_IMAGE = "/destination/placeholder.jpg";

/* The four cruise-styled entries below are not real countries — no CMS
   `countries` record or region-grid card exists for them — so they used to
   fall through to FALLBACK_IMAGE and each showed an unrelated design still
   (a Japanese pagoda for "China Cruise", the Matterhorn for "Mediterranean",
   …). Matched stock photos here until an editor adds real uploads. */
const CRUISE_FALLBACK_IMAGES = {
  "china-cruise": "/destinations/cruise/china-cruise.avif",
  "royal-caribbean": "/destinations/cruise/royal-caribbean.avif",
  mediterranean: "/destinations/cruise/mediterranean.avif",
  "cordelia-cruise": "/destinations/cruise/cordelia-cruise.avif",
};

/* The cruise style's sailings until an editor picks its countries in the CMS. */
const FEATURED_CRUISES = [
  { slug: "china-cruise", name: "China Cruise", tagline: "Yangtze • Coastal routes", region: "asia" },
  { slug: "royal-caribbean", name: "Royal Caribbean", tagline: "Islands • Ocean luxury", region: "caribbean" },
  { slug: "mediterranean", name: "Mediterranean", tagline: "Historic ports & coasts", region: "asia" },
  { slug: "cordelia-cruise", name: "Cordelia Cruise", tagline: "Coastal voyages • Open waters", region: "north-america" },
];
...
async function fetchPublishedCountries() {
  try {
    const json = await strapiFetchAll("countries", {
      query: {
        fields: ["slug", "continentSlug"],
        populate: {
          heroSection: { populate: { image: true } },
          packagesSection: {
            populate: { packages: { fields: ["title", "link"] } },
          },
        },
      ...
      .map((entry) => ({
        ...
        packages: list(entry.packagesSection?.packages)
          .map((pkg) => ({
            ...
            // The package page's own slug, the last segment of the card's link.
            slug: text(pkg.link, "").split("/").filter(Boolean).pop() ?? "",
          }))
...
/* Every package title the published countries sell, as { value, label,
   keys, packageSlugs }: the slug the URL carries, the title that prints, the
   country keys it belongs to and the package pages its cards link to. A title
   sold under two countries becomes one option matching both. Alphabetical, so
   the dropdown reads the same on every visit. */
function collectPackages(published) {
  const byValue = new Map();

  for (const country of published) {
    for (const { title, slug } of country.packages) {
      ...
      if (slug) entry.packageSlugs.add(slug);
...
export async function getSearchResults({ pkg, ...filters } = {}) {
  /* Not swallowed like the two reads beside it: those only cost pictures and
     links, while a failed read here would list the countries the editor left
     out. */
  const [published, cardImages, picked, pickedPackages] = await Promise.all([
    fetchPublishedCountries(),
    fetchRegionCardImages(),
    getTravelStyleDestinations(filters.style),
    getTravelStylePackages(filters.style),
  ]);
  ...
  const packageOption = pkg
    ? collectPackages(published).find((p) => p.value === pkg)
    : undefined;
  const packageKeys = pkg ? new Set(packageOption?.keys ?? []) : undefined;

  // A package page's region, from the country its URL sits under (dubai → asia).
  const homeRegions = new Map(
    published
      .filter((entry) => entry.canonical)
      .map((entry) => {
        const cut = entry.key.indexOf("/");
        return [entry.key.slice(cut + 1), entry.key.slice(0, cut)];
      }),
  );
  ...
  const pictureFor = (keys, fallback) =>
    keys.map((key) => heroImages.get(key)).find(Boolean) ??
    keys.map((key) => cardImages.get(key)).find(Boolean) ??
    fallback;

  /* What the editor picked on the travel style decides the list. A style
     with nothing picked keeps lib/tripTypes.js, or the four sailings for
     cruise (which never took the package filter), so the page does not empty
     while the CMS is being filled in. */
  let matches;
  if (picked || pickedPackages.length) {
    matches = [
      ...(picked ? pickStyleCountries(picked, { ...filters, packageKeys }) : []),
      ...pickedPackages
        .filter(
          (pack) =>
            packageMatches(
              { ...pack, region: homeRegions.get(pack.destination) },
              filters,
            ) && (!pkg || Boolean(packageOption?.packageSlugs.has(pack.slug))),
        )
        .map((pack) => ({ pack })),
    ];
  } else if (filters.style === "cruise") {
    matches = FEATURED_CRUISES.filter((cruise) =>
      entryMatches(cruise, filters),
    ).map((entry) => ({ entry }));
  } else {
    matches = filterCountries({ ...filters, packageKeys }).map((country) => ({
      country,
    }));
  }

  return matches.map(({ country, entry, pack }, index) => {
    const fallback = FALLBACK_IMAGE;

    if (pack) {
      return {
        // Prefixed: a package slug can equal a country slug (delhi-agra-jaipur).
        slug: `package:${pack.destination}/${pack.slug}`,
        name: pack.name,
        description: pack.duration ?? pack.tagline ?? "",
        image: pack.image ?? fallback,
        href: `/destinations/${pack.destination}/${pack.slug}`,
      };
    }
    ...
    if (entry) {
      return {
        slug: entry.slug,
        name: entry.name,
        description: [entry.tagline, regionLabel(entry.region)]
          .filter(Boolean)
          .join(" — "),
        image: pictureFor(
          [`${entry.region}/${entry.slug}`],
          CRUISE_FALLBACK_IMAGES[entry.slug] ?? fallback,
        ),
        href: `/${entry.region}/${entry.slug}`,
      };
    }
    ...
    return {
      ...
      description: [country.tagline, country.regions[0].label]
        ...
        .join(" — "),
      image: pictureFor(
        country.regions.map((region) => `${region.key}/${country.slug}`),
        fallback,
      ),

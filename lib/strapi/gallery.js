...
import { mediaAlt, mediaOrientation, mediaUrl } from "./media";
...
function normaliseGallery(entry) {
  const photos = list(entry.photos).map((photo, index) => ({
    ...
    /* Shape of the tile, read off the upload's own dimensions so the mosaic
       lays a portrait photo into a tall frame and a landscape one into a wide
       frame rather than cropping every picture to one ratio. There is no CMS
       field for it — Strapi already stores width and height — so nothing in the
       panel has to change. A missing upload has no size and falls to
       "landscape", which is also what FALLBACK_PHOTOS above are. */
    orientation: mediaOrientation(photo.image),

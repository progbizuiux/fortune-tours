...
export function destinationNameForSlug(slug) {
  ...
  return Object.keys(DESTINATION_PAGES).find(
...
export const DESTINATION_REGIONS = RAW_DESTINATION_REGIONS.map((region) => ({
  ...
  countries: region.countries.map((c) => ({
    ...
    href: `/${region.key}/${countrySlug(c.name)}`,
  })),
}));
...
export function resolveCountryHref(place, region, published) {
  ...
  const elsewhere = liveCountryHref(
    REGIONS_BY_COUNTRY.get(place.name) ?? [],
    place.slug,
    published,
  );

  return elsewhere ?? destinationPageHref(place.name) ?? region.href;
}
